...
def test_ensure_cluster_prereqs_reuses_existing_managed_local_postgres_forward(
    ...
    monkeypatch.setattr(module, "_listening_pids", lambda port: [4242])

    def fake_process_command_line(pid: int) -> str:
    ...
    monkeypatch.setattr(module, "_process_command_line", fake_process_command_line)
    monkeypatch.setattr(
        module.subprocess,
        "Popen",
...
def test_ensure_cluster_prereqs_reuses_matching_unmanaged_local_postgres_forward(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    module = _load_run_infra_loops_module()
    ...
    monkeypatch.setattr(module, "port_is_open", lambda host, port: True)
    monkeypatch.setattr(module, "_existing_port_forward_pid", lambda: 4242)
    monkeypatch.setattr(module, "_listening_pids", lambda port: [5151])
    monkeypatch.setattr(
        module,
...
def test_ensure_cluster_prereqs_rejects_stale_managed_pid_with_conflicting_listener(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    module = _load_run_infra_loops_module()
    policy = load_loop_policy(Path("ops/infra-loop-policy.toml"))
    ...
    monkeypatch.setattr(module, "port_is_open", lambda host, port: True)
    monkeypatch.setattr(module, "_existing_port_forward_pid", lambda: 4242)
    monkeypatch.setattr(module, "_listening_pids", lambda port: [6161])
    monkeypatch.setattr(
        module,
        "_process_command_line",
        lambda pid: "/usr/bin/postgres -D /tmp/conflicting-db",
    )
    monkeypatch.setattr(
        module.subprocess,
        "Popen",
    ...
    with pytest.raises(
        RuntimeError,
        match=(
            r"Recorded managed Postgres port-forward pid 4242 is still running "
            r"but does not own the active listener on 127\.0\.0\.1:5432"
        ),
    ):
        module._ensure_cluster_prereqs(policy=policy, run_id="run-1")
